...
function validateTesting(config) {
    var testing = config.testing = config.testing || {};
    ...
    if (!Array.isArray(testing.testPathIgnorePatterns)) {
        testing.testPathIgnorePatterns = DEFAULT_IGNORE_PATTERNS.map(function (ignorePattern) {
            return util_1.pathJoin(config, testing.rootDir, ignorePattern);
        });
        config.outputTargets.filter(function (o) { return (o.type === 'dist' || o.type === 'www') && o.dir; }).forEach(function (outputTarget) {
...
var DEFAULT_TS_TRANSFORM = '^.+\\.(ts|tsx)$';
var DEFAULT_MODULE_FILE_EXTENSIONS = [
    'ts',
    'tsx',
    'js',
    'json'
