...
function validateNamespace(config) {
    config_utils_1.setStringConfig(config, 'namespace', DEFAULT_NAMESPACE);
    ...
    config_utils_1.setStringConfig(config, 'fsNamespace', config.namespace.toLowerCase());
    if (config.namespace.includes('-')) {
        // convert to PascalCase
        // this is the same namespace that gets put on "window"
        config.namespace = helpers_1.dashToPascalCase(config.namespace);
    }
}
