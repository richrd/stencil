"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var app_file_naming_1 = require("./app-file-naming");
function createAppRegistry(config) {
    // create the shared app registry object
    var appRegistry = {
        namespace: config.namespace,
        fsNamespace: config.fsNamespace,
        loader: "../" + app_file_naming_1.getLoaderFileName(config)
    };
    return appRegistry;
}
exports.createAppRegistry = createAppRegistry;
function getAppRegistry(config, compilerCtx, outputTarget) {
    var registryJsonFilePath = app_file_naming_1.getRegistryJson(config, outputTarget);
    var appRegistry;
    try {
        // open up the app registry json file
        var appRegistryJson = compilerCtx.fs.readFileSync(registryJsonFilePath);
        // parse the json into app registry data
        appRegistry = JSON.parse(appRegistryJson);
    }
    catch (e) {
        throw new Error("Error parsing app registry, " + registryJsonFilePath + ": " + e);
    }
    return appRegistry;
}
exports.getAppRegistry = getAppRegistry;
function serializeComponentRegistry(cmpRegistry) {
    var appRegistryComponents = {};
    Object.keys(cmpRegistry).sort().forEach(function (tagName) {
        var cmpMeta = cmpRegistry[tagName];
        appRegistryComponents[tagName] = {
            bundleIds: cmpMeta.bundleIds
        };
        if (cmpMeta.encapsulationMeta === 1 /* ShadowDom */) {
            appRegistryComponents[tagName].encapsulation = 'shadow';
        }
        else if (cmpMeta.encapsulationMeta === 2 /* ScopedCss */) {
            appRegistryComponents[tagName].encapsulation = 'scoped';
        }
    });
    return appRegistryComponents;
}
exports.serializeComponentRegistry = serializeComponentRegistry;
function writeAppRegistry(config, compilerCtx, buildCtx, outputTarget, appRegistry, cmpRegistry) {
    return __awaiter(this, void 0, void 0, function () {
        var registryJson, appRegistryWWW, relPath;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (buildCtx.hasError || !buildCtx.isActiveBuild) {
                        return [2 /*return*/];
                    }
                    if (!(outputTarget.type === 'www')) return [3 /*break*/, 2];
                    appRegistry.components = serializeComponentRegistry(cmpRegistry);
                    registryJson = JSON.stringify(appRegistry, null, 2);
                    // cache so we can check if it changed on rebuilds
                    compilerCtx.appFiles.registryJson = registryJson;
                    appRegistryWWW = app_file_naming_1.getRegistryJson(config, outputTarget);
                    return [4 /*yield*/, compilerCtx.fs.writeFile(appRegistryWWW, registryJson)];
                case 1:
                    _a.sent();
                    relPath = config.sys.path.relative(config.rootDir, appRegistryWWW);
                    buildCtx.debug("writeAppRegistry: " + relPath);
                    _a.label = 2;
                case 2: return [2 /*return*/];
            }
        });
    });
}
exports.writeAppRegistry = writeAppRegistry;
