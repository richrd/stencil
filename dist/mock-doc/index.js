...
const cssProxyHandler = {
    ...
    set: function (cssStyle, prop, value) {
        if (prop in cssStyle) {
            cssStyle[prop] = value;
        }
        else {
...
function parseFragmentUtil(ownerDocument, html) {
    const frag = parseFragment(html.trim(), getParser(ownerDocument));
    return frag;
}
function getParser(ownerDocument) {
    ...
    const treeAdapter = {
        ...
        insertTextBefore(parentNode, text, referenceNode) {
            const prevNode = parentNode.childNodes[parentNode.childNodes.indexOf(referenceNode) - 1];
            if (prevNode && prevNode.nodeType === 3 /* TEXT_NODE */) {
...
function parseSelector(subselects, selector, options){
	...
	function stripWhitespace(start){
		while(isWhitespace$1(selector.charAt(start))) start++;
...
function serializeToHtml(node, opts, output) {
    if (node.nodeType === 1 /* ELEMENT_NODE */) {
        ...
        if (!ignoreTag) {
            ...
            if (opts.pretty) {
                node.attributes.items.sort((a, b) => {
                    if (a.name < b.name)
                        return -1;
                    if (a.name > b.name)
                        return 1;
            ...
            for (let i = 0, attrsLength = node.attributes.items.length; i < attrsLength; i++) {
                const attr = node.attributes.items[i];
            ...
            if (node.hasAttribute('style')) {
                output.text.push(` style="${opts.minifyInlineStyles ? (node.style.cssTextMinified || node.style.cssText) :
                    (node.style.cssText)}">`);
            }
            else {
                output.text.push('>');
        ...
        if (typeof node.nodeValue === 'string' && node.nodeValue.trim() !== '') {
            ...
            const parentTagName = (node.parentNode && node.parentNode.nodeType === 1 /* ELEMENT_NODE */ ? node.parentNode.nodeName.toLowerCase() : null);
    ...
    else if (node.nodeType === 8 /* COMMENT_NODE */ && !opts.removeHtmlComments) {
        if (opts.newLines) {
            output.text.push('\n');
        }
        if (opts.indentSpaces > 0) {
...
const AMP_REGEX$1 = /&/g;
const NBSP_REGEX$1 = /\u00a0/g;
const DOUBLE_QUOTE_REGEX$1 = /"/g;
const LT_REGEX$1 = /</g;
const GT_REGEX$1 = />/g;
const CAN_REMOVE_ATTR_QUOTES = /^[^ \t\n\f\r"'`=<>\/\\-]+$/;
function escapeString(str, attrMode) {
    str = str.replace(AMP_REGEX$1, '&amp;').replace(NBSP_REGEX$1, '&nbsp;');
    if (attrMode) {
        return str.replace(DOUBLE_QUOTE_REGEX$1, '&quot;');
    }
    return str.replace(LT_REGEX$1, '&lt;').replace(GT_REGEX$1, '&gt;');
}
...
class MockElement extends MockNode {
    ...
    get nextElementSibling() {
        ...
        if (parentElement && (parentElement.nodeType === 1 /* ELEMENT_NODE */ || parentElement.nodeType === 11 /* DOCUMENT_FRAGMENT_NODE */ || parentElement.nodeType === 9 /* DOCUMENT_NODE */)) {
...
function getElementsByName(elm, name, foundElms) {
    const children = elm.children;
    for (let i = 0; i < children.length; i++) {
        const childElm = children[i];
        if (childElm.name && childElm.name.toLowerCase() === name) {
            foundElms.push(childElm);
        }
...
class MockLocation {
    ...
    set href(value) {
        ...
        this.protocol = url$$1.protocol;
        this.host = url$$1.host;
        this.port = url$$1.port;
        this.pathname = url$$1.pathname;
        this.search = url$$1.search;
        this.hash = url$$1.hash;
        this.username = url$$1.username;
        this.password = url$$1.password;
        this.origin = url$$1.origin;
    }
    assign(_url) {
        //
    }
...
class MockWindow {
    ...
    get document() {
        if (!this._document) {
            this._document = new MockDocument();
            this._document.defaultView = this;
        }
...
function applyWindowToGlobal(global) {
    global._window = null;
...
exports.applyWindowToGlobal = applyWindowToGlobal;
